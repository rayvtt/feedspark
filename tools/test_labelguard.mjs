...

{
  ...
  // be cumulated or counted toward the score"). Colour / gender / age_group / size / material /
  // pattern are scored over the products whose Google category asks for them, weighted by the
  // share of the catalogue that is, and not at all where no product does.
  console.log('\n— GPC category scope: an attribute scores only where Google asks for it —');
  const C = (v) => LG.gpcClass(v);
  const inS = (k, v) => LG.gpcInScope(k, C(v));
  // every language + form the live estate actually ships (probed 23 Sep 2026)
  ok('en-GB (Reiss / Monsoon / Schuh / Superdry GB) reads as clothing', inS('size', 'Clothing & Accessories > Clothing > Dresses') && inS('color', 'Clothing & Accessories > Clothing Accessories > Belts'));
  ok('en-US (Reiss US), de-DE, fr-FR, nl-NL, da-DK, es-ES read as clothing too',
    ['Apparel & Accessories > Clothing > Outerwear > Coats & Jackets', 'Bekleidung & Accessoires > Bekleidung > Röcke', 'Vêtements et accessoires > Vêtements > Hauts',
      'Kleding en accessoires > Kleding > Jurken', 'Tøj og tilbehør > Tøj > Bukser', 'Ropa y accesorios > Prendas de vestir > Camisas y tops'].every((v) => inS('size', v)));
  ok('shoes carry size in any language (Schuh DE "Bekleidung & Accessoires > Schuhe")', inS('size', 'Bekleidung & Accessoires > Schuhe') && inS('size', 'Clothing & Accessories > Shoes'));
  ok('a stray leading "{" (Reiss CZ/FI/KW/RO/SE/SG/SK) does not hide a clothing product', inS('material', '{Clothing & Accessories > Clothing > Outerwear > Coats & Jackets'));
  ok('a numeric GPC reads the same as its path', inS('size', '1604') && !inS('size', '3237') && inS('color', '5123'));
  ok('craft, bags and pet supplies are outside every clothing rule',
    ['Arts & Entertainment > Hobbies & Creative Arts > Arts & Crafts', 'Luggage & Bags > Backpacks', 'Animals & Pet Supplies > Pet Supplies', 'Heim & Garten > Dekoration']
      .every((v) => ['color', 'gender', 'age_group', 'size', 'material', 'pattern'].every((k) => !inS(k, v))));
  ok('accessories are asked colour / gender / material but not size (size is Clothing + Shoes only)',
    inS('color', 'Clothing & Accessories > Jewellery & Watches > Rings') && inS('gender', 'Clothing & Accessories > Jewellery & Watches > Rings') && !inS('size', 'Clothing & Accessories > Jewellery & Watches > Rings'));
  ok('Google’s gender exemptions hold: Watch Bands (5123) carry colour, not gender or age_group',
    inS('color', 'Clothing & Accessories > Jewellery & Watches > Watch Accessories > Watch Bands') && !inS('gender', 'Clothing & Accessories > Jewellery & Watches > Watch Accessories > Watch Bands') && !inS('age_group', 'Apparel & Accessories > Jewelry > Watch Accessories > Watch Bands'));
  ok('Cufflinks (193) are exempt from age_group only', inS('gender', 'Apparel & Accessories > Clothing Accessories > Cufflinks') && !inS('age_group', 'Apparel & Accessories > Clothing Accessories > Cufflinks'));
  ok('a BLANK or unreadable category stays IN scope — the scope narrows only on what the feed says', C('') === null && C('Clothing') === null && inS('size', '') && inS('gender', 'totally made up'));
  ok('every taxonomy file the table was built from is Google’s (26 locales, IDs asserted by the generator)', /generated by tools\/build_gpc_scope\.py from Google's taxonomy-with-ids files \(26 locales/.test(readFileSync(srcPath, 'utf8')));

  // the collector counts per product, on the stream the agent and the page both use
  const { createRequire } = await import('node:module');
  const FA = createRequire(import.meta.url)('../docs/feedlab_engine.js');
  const it = (id, gpc, extra) => '<item><g:id>' + id + '</g:id><g:title>T</g:title><g:google_product_category>' + gpc + '</g:google_product_category>' +
    Object.entries(extra).map(([k, v]) => '<g:' + k + '>' + v + '</g:' + k + '>').join('') + '</item>';
  let xml = '<?xml version="1.0"?><rss xmlns:g="http://base.google.com/ns/1.0"><channel>';
  for (let i = 0; i < 90; i++) xml += it('y' + i, 'Arts &amp; Entertainment &gt; Hobbies &amp; Creative Arts &gt; Arts &amp; Crafts', i < 40 ? { material: 'Acrylic', color: 'Red' } : { color: 'Blue' });
  for (let i = 0; i < 8; i++) xml += it('d' + i, 'Clothing &amp; Accessories &gt; Clothing &gt; Dresses', i < 6 ? { material: 'Cotton', color: 'Navy', size: '10' } : { color: 'Navy', size: '12' });
  for (let i = 0; i < 2; i++) xml += it('r' + i, 'Clothing &amp; Accessories &gt; Jewellery &amp; Watches &gt; Rings', { color: 'Gold' });
  xml += '</channel></rss>';
  const col = LG.xmlCollector({ client: 'Hobbycraft', market: 'gb' });
  const pr = FA.createXmlParser(col.onRow);
  for (let o = 0; o < xml.length; o += 377) pr.push(xml.slice(o, o + 377));
  pr.end();
  const at = col.finish().snap.attrs;
  eq('material: scored over the 10 clothing products (6 filled), not the 100 in the feed', [at.material.cov, at.material.all, at.material.scope.n, at.material.scope.f, at.material.scope.t], [60, 46, 10, 6, 100]);
  eq('size: asked of the 8 dresses only (rings are accessories)', [at.size.cov, at.size.scope.n], [100, 8]);
  eq('gender: no column, 10 clothing products — a known requirement, scored as absent', [at.gender.present, at.gender.scope.n, !!at.gender.na], [false, 10, false]);
  ok('pattern: no column AND a scope — reads as needed on the 10, never as not applicable', !at.pattern.present && at.pattern.scope.n === 10 && !at.pattern.na);

  // the score: not applicable drops out, a small scope weighs small
  const base = { id: { present: true, cov: 100 }, title: { present: true, cov: 100 }, description: { present: true, cov: 100 }, link: { present: true, cov: 100 },
    image_link: { present: true, cov: 100 }, availability: { present: true, cov: 100 }, price: { present: true, cov: 100 }, gtin: { present: true, cov: 100 } };
  const withA = (x) => Object.assign({}, JSON.parse(JSON.stringify(base)), x);
  const craft = withA({ material: { present: true, cov: 48, all: 48, scope: { n: 0, f: 0, u: 0, t: 24000 }, na: true } });
  // the reference: the same feed with material waived outright
  const without = LG.goldenScore(withA({}), { expected: [], waived: ['material'] }).score;
  eq('an attribute no product calls for is not in the score — even when the profile stars it',
    [LG.goldenScore(craft).score, LG.goldenScore(craft, { expected: ['material'], waived: [] }).score], [without, without]);
  const tiny = withA({ material: { present: true, cov: 0, all: 0, scope: { n: 2, f: 0, u: 0, t: 24000 } } });
  ok('two clothing products in 24,000 cannot drag the score (weight × 2/24,000)', Math.abs(LG.goldenScore(tiny).score - without) <= 0.1, [LG.goldenScore(tiny).score, without]);
  ok('…where the whole-catalogue reading of the same 0% would have cost points', LG.goldenScore(withA({ material: { present: true, cov: 0 } })).score < without - 1);
  const whole = withA({ material: { present: true, cov: 0, scope: { n: 24000, f: 0, u: 0, t: 24000 } } });
  const legacy = withA({ material: { present: true, cov: 0 } });
  eq('an all-apparel feed scores exactly as before the scope existed', LG.goldenScore(whole).score, LG.goldenScore(legacy).score);
  const gap = withA({ gender: { present: false, scope: { n: 500, f: 0, u: 0, t: 1000 } } });
  ok('a cond attribute ABSENT on products Google requires it for is a gap even without a profile', LG.goldenScore(gap).score < 100 && LG.goldenScore(gap).condMissing.includes('gender'));
  ok('…and an absent attribute nobody needs is not listed missing', !LG.goldenScore(withA({ gender: { present: false, na: true, scope: { n: 0, f: 0, u: 0, t: 9 } } })).condMissing.includes('gender'));
  const idx = LG.goldenCovIndex(craft);
  ok('the estate index stores null coverage for not-applicable (the Playbook skips it) and the in-scope count', idx.cov.material === null && idx.sc.material === 0 && idx.cov.title === 100 && !('title' in idx.sc));

  // alerts never read a change of basis as a drop
  const old = { attrs: { size: { present: true, cov: 62.8 } } };
  const now = { attrs: { size: { present: true, cov: 40, scope: { n: 10, f: 4, u: 0, t: 100 } } } };
  ok('the first scoped scan after a whole-catalogue baseline raises no drop alert', LG.diffCoverage(old, now).length === 0);
  ok('a column vanishing where no product needs it raises nothing', LG.diffCoverage({ attrs: { size: { present: true, cov: 60 } } }, { attrs: { size: { present: false, na: true, scope: { n: 0, f: 0, u: 0, t: 9 } } } }).length === 0);
  ok('a real drop on the same basis still alerts', LG.diffCoverage({ attrs: { size: { present: true, cov: 99, scope: { n: 10, f: 10, u: 0, t: 10 } } } }, now).some((a) => a.code === 'attr-drop'));

  // the page's re-score is the engine's, on the same inputs
  ...
  const twin = new Function(cut('  var SPEC = [', '  // the category each scoped') + cut('  function scopeShare(a)', '  // industry benchmark from the estate index') +
    cut('  function attrsFromCov(cov, sc, rows)', '  function rescoreEstate()') + '; return { goldenScore: goldenScore, attrsFromCov: attrsFromCov };')();
  const profs = [null, { expected: ['material', 'pattern'], waived: ['size_type'] }, { expected: ['color', 'size', 'gender', 'age_group', 'item_group_id'], waived: [] }];
  const cases = [craft, tiny, whole, gap, at, withA({ size: { present: true, cov: 70, scope: { n: 300, f: 210, u: 5, t: 1000 } }, pattern: { present: false, scope: { n: 300, f: 0, u: 0, t: 1000 } } })];
  ok('the /golden page scores every case exactly as the engine does (6 feeds × 3 profiles)',
    cases.every((a) => profs.every((p) => twin.goldenScore(a, p) === LG.goldenScore(a, p).score)));
  const round = (a, rows) => { const i = LG.goldenCovIndex(a); return twin.attrsFromCov(i.cov, i.sc, rows); };
  ok('…and the estate re-score (index → attrs → score) lands on the scan-time number',
    [[craft, 24000], [tiny, 24000], [gap, 1000], [at, 100]].every(([a, rows]) => profs.every((p) => twin.goldenScore(round(a, rows), p) === LG.goldenScore(a, p).score)));
  ok('page: a not-applicable row says so, carries no ask/brief actions, and the coverage names what it is a share of',
    /no ' \+ esc\(gs\) \+ ' products — not scored/.test(gr) && /if \(na\) \{ h \+= row \+ '<span class="at-act"><\/span><\/div>'; return; \}/.test(gr) && /'of the ' \+ fmt\(a\.scope\.n\) \+ ' ' \+ gs \+ ' product'/.test(gr));
  ok('page: the Δ column never differences a scoped reading against a whole-catalogue reference', /!!refA\.scope !== !!a\.scope\) delta = /.test(gr));
  const wk5 = readFileSync(new URL('../cloudflare/feedspark-deck/src/worker.js', import.meta.url), 'utf8');
