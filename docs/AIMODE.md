...
## Every saved line in the breakdown

Ray, 21 Sep 2026, sending the expanded ⌄ row of a Reiss GB quote (dashboard, feeds, retainer hours,
setup hours, the discount, the two totals): *"on Tracker, each quote breakdown should have every line
saved as this example."*

The ⌄ row listed the legacy per-SKU fields, the extra lines, the new-products bundle, the setup fee and
the discount — and not the Spark AI attributes, the lines most quotes are now made of, so a Spark AI
quote's breakdown read as a total with nothing under it. `aimDetailRows(q)` now renders them between the
extra lines and the bundle: one caption row naming the basis (attributes · units and the unit · the
arrival cohort · new a month, with the Spark AI subtotal one-off + monthly), then one row per attribute —
its label, the route it is generated by (the one `AIM_ROUTE_LABEL` table; the AI floor reads *monthly
minimum*), the saved detail sentence, and its money: one-off with the hours, monthly, or the saved
*Included* note when the scrape run above already covers it.

They are the **snapshot's** lines (`q.aim.lines`, frozen at save), never re-priced from the live rate
card — the tracker shows what finance signed off, the same rule the rest of the row follows. Every
